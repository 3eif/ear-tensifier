				...
				if (res.loadType == 'TRACK_LOADED') {
					...
					client.music.search(searchQuery, message.author).then(async res2 => {
						if (res2.loadType == 'TRACK_LOADED') {
							...
							if (!playlist && msg) msg.edit(`**${res2.tracks[0].title}** (${moment.duration(res2.tracks[0].length, 'milliseconds').format('mm:ss', { trim: false })}) has been added to the queue by **${res2.tracks[0].requester.tag}**`);
							if (!player.playing && !player.paused && !player.queue.length) player.play();
							resolve();
						}
							...
							msg.edit(`**${res2.playlist.info.name}** (${moment.duration(res2.playlist.tracks.reduce((acc, cure) => ({ duration: acc.length + cure.length })).duration, 'milliseconds').format('mm:ss', { trim: false })}) (${res2.playlist.tracks.length} tracks) has been added to the queue by **${res2.playlist.tracks[0].requester.tag}**`);
							resolve();
						}
