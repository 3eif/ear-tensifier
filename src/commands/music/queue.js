	...
	async run(client, message, args) {
		...
		if (player.queue.length > 10) {
			if(args[0]) {
				...
				const queueStr2 = `${player.queue.slice(pageStart, pageEnd).map(song => `**${index2++}.** [${song.title}](${song.uri}) \`[${client.formatDuration(song.duration)}]\` • <@${!song.requester.id ? song.requester : song.requester.id ? song.requester : !song.requester.id ? song.requester : song.requester.id}>`).join('\n')}`;
				...
				for (let i = 0; i < pagesNum; i++) {
					const str = `${player.queue.slice(i * 10, i * 10 + 10).map(song => `**${n++}.** [${song.title}](${song.uri}) \`[${client.formatDuration(song.duration)}]\` • <@${!song.requester.id ? song.requester : song.requester.id}>`).join('\n')}`;
