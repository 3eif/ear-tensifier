...
module.exports = class Skipto extends Command {
	...
	async run(client, message, args) {
		...
		if ((args[0] > player.queue.tracks.length) || (args[0] && !player.queue.tracks[args[0] - 1])) return message.channel.send('Song not found.');
		const { title } = player.queue.tracks[args[0] - 1];
		if (args[0] == 1) player.stop();
		player.queue.tracks.splice(0, args[0] - 1);
		player.stop();
