...
module.exports = class Move extends Command {
	...
	async run(client, message, args) {
		...
		if ((args[0] > player.queue.length) || (args[0] && !player.queue.tracks[args[0] - 1])) return message.channel.send('Song not found.');

		if (!args[1]) {
			const song = player.queue.tracks[args[0] - 1];
			player.queue.tracks.splice(args[0] - 1, 1);
			player.queue.tracks.splice(0, 0, song);
			...
			if ((args[1] > player.queue.length) || !player.queue.tracks[args[1] - 1]) return message.channel.send('Song not found.');
			const song = player.queue.tracks[args[0] - 1];
			player.queue.tracks.splice(args[0] - 1, 1);
			player.queue.tracks.splice(args[1] - 1, 0, song);
			return message.channel.send(`Moved **${song.title}** to position: **${args[1]}**.`);
		}
	}
};