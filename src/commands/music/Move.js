...
module.exports = class Move extends Command {
    ...
    async run(client, ctx, args) {
        ...
        if ((args[0] > player.queue.length) || (args[0] && !player.queue[args[0]])) return ctx.sendMessage('Song not found.');
        ...
        if (!args[1]) {
            ...
            if ((args[1] > player.queue.length) || !player.queue[args[1]]) return ctx.sendMessage('Song not found.');
            const song = player.queue[args[0] - 1];
            player.queue.splice(args[0] - 1, 1);
