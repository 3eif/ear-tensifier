...
module.exports = class VoiceStateUpdate extends Event {
    ...
    async run(oldMember, newMember) {
        ...
        if (oldMember.guild.members.cache.get(this.client.user.id).voiceChannel === oldMember.voiceChannel) {
            if (oldMember.guild.members.cache.get(this.client.user.id).voice.channel.members.size == null || (oldMember.guild.members.cache.get(this.client.user.id).voice.channel.members.size - 1) === 0) {
                ...
                msg.edit(`I left ${emojis.voice}\`${oldMember.guild.members.cache.get(this.client.user.id).voice.channel.name}\` becayse I was left alon.`)
            }
        }
    };
}